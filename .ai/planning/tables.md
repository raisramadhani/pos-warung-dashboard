...
| Soft deletes | `TrashedFilter` | https://filamentphp.com/docs/5.x/tables/filters/trashed |
...
Actions can be placed in the table header or toolbar. Important:
`toolbarActions()` overrides `bulkActions()`. To preserve bulk actions when
using toolbar actions, wrap them in a `BulkActionGroup`:
